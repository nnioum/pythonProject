# Программа загдывает слово и просит ввести букву
# Если буква есть в слове, просит ввести другую букву, иначе добавляеться палочка к рисунку
# Игра заканчиваеться в двух случаях:
# 1. угадываешь слово
# 2. заканчиваеться рисунок
print('Давайте играть в угадайку слов!')
print('Правило игры:', 'Освоить принцип «Виселицы» смогут даже дети.'
                           ' Главное, чтобы вы умели читать и писать, составлять слова.'
                           ' Я загадываю слово, а именно существительное.'
                           ' Хитрость в том, чтобы «повесить» игрока до того, как он угадает слово. '
                           'На этапе подготовки я рисую виселицу.'
                           ' «Палач», а точнее я, загадываю слово и под эшафотом рисую свободные пустые клеточки.'
                           ' В них убудут вписываться буквы загаданного слова.'
                           ' Каждый ошибочный ход приближает вас к казни.'
                           ' Постепенно на виселице появляется: веревка, голова, шея, туловище, руки, ноги.', sep='\n')
import random
def play(word_list):
    ...
    tries = 9
    print('Удачной игры!!! _0_0_')
    print(display_hangman(tries))
    print(word_completion)
    while True:
        s = input('Введите букву - ').upper()
        if len(s) > 1 and s != word:
            print('Если вы хотели ввести загаданное слово, то она неверное')
            continue
        if s.isdigit() == True:
        ...
        if s not in word:
            ...
            print('В этом слове этой буквы нету. Вы теряете жизнь')
            tries -= 1
            print(display_hangman(tries))
            print(word_completion)
            if tries == 0:
                print('К сожелению, вы исчерпали все свои попытки')
                print('Загадонное слово -', word)
                break
        if s in word:
            print('В этом слове эта буква есть. Молодец!')
            guessed_words.append(s)
...
def pop():
    ...
    list2 = ['нет', '0', 'no']
    if a in list1:
        print('Тогда давайте продолжим!))', 'И не забудем вас еще раз сказать!', sep='\n', end=' ')
        play(word_list)
    if a in list2:
        print('Удачи! Скорее бы увидеться')
